...
import { User, AccountStatus, StatusType } from '../types/User';
...
type AccountStatusFilter = '' | StatusType;
...
    try {
      const result = await adminService.updateAccountStatus(personId, AccountStatus.Active); 
...
      try {
        const result = await adminService.updateAccountStatus(personId, AccountStatus.Suspended); 
...
    { label: 'Suspendidos', value: AccountStatus.Suspended, count: users.filter(u => u.estado_cuenta === AccountStatus.Suspended).length },
    { label: 'Reportados', value: AccountStatus.Reported, count: users.filter(u => u.estado_cuenta === AccountStatus.Reported).length },
  ];
...
                <button
                  key={filter.value}
                  onClick={() => setStatusFilter(filter.value as AccountStatusFilter)}
...
                            </td>
                            <td className="py-4 px-4">
                              {userItem.estado_cuenta === AccountStatus.Unverified ? (
...
                                  </button>
                                </div>
                              ) : userItem.estado_cuenta === AccountStatus.Active ? (
                                <button
                                    onClick={() => handleSuspend(userItem.persona_id)}
                                    className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white hover:opacity-90 transition-opacity flex items-center gap-1"
                                    style={{ backgroundColor: '#EF4444' }}
...
              {users.filter(u => u.estado_cuenta === AccountStatus.Unverified).length}
            </p>
          </div>
...
            </div>
            <p className="text-3xl font-bold text-gray-900">
              {users.filter(u => u.estado_cuenta === AccountStatus.Active).length}
            </p>
          </div>
        </div>
      </main>
